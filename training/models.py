...
from environment.pipeline import algorithmic_step
...
def pyc_to_torch(
    d: dict[Union[str, object], float],
    s_indices: List[Union[str, object]],
    a_indices: List[object] = None,
):
    ...
    for k in d:
        if k in ("reward", "action"):
            continue
        data_array[s_indices.index(k)] = d[k]
...
def torch_to_pyc(t: torch.Tensor, indices: List[object]):
    t = t.squeeze()
    if len(indices) != t.size(-1):
        t = t[:-1]
    return {k: t[i].item() for i, k in enumerate(indices)}
...
class GRULayer(myModule):
    ...
    def __init__(self, input_size, hidden_size, bias=True, norm=False):
        ...
        self.input_to_reset = (
            ...
            if norm
            else nn.Linear(input_size, hidden_size, bias=bias)
        )
        self.hidden_to_reset = (
            nn.Sequential(
                nn.Linear(hidden_size, hidden_size, bias=bias),
                nn.LayerNorm(hidden_size),
            )
            if norm
            else nn.Linear(hidden_size, hidden_size, bias=bias)
        )

        self.input_to_update = (
            nn.Sequential(
                nn.Linear(input_size, hidden_size, bias=False),
                nn.LayerNorm(hidden_size),
            )
...
class DQN(myModule):
    def __init__(
        ...
        super().__init__()
        base_in_size = (
            len(state_keys) if p_mode == "clarion" else (72 if p_mode == "asis" else 32)
        )
        n_observations = base_in_size + (2 if a2c and feed_state else 0)
        n_actions = len(action_keys) + int(a2c)
        network = [("fc0", nn.Linear(n_observations, 128)), ("rfc0", nn.ReLU())]
        assert n_layers % 2 == 0, "Must be even number"
        last_num = 128
        for n_layer in range(n_layers):
            if n_layer < n_layers // 2:
                network.append((f"fc{n_layer + 1}", nn.Linear(last_num, last_num * 2)))
                last_num *= 2
            else:
                network.append((f"fc{n_layer + 1}", nn.Linear(last_num, last_num // 2)))
                last_num //= 2
            if norm:
                network.append((f"ln{n_layer + 1}", nn.LayerNorm(last_num)))
            network.append((f"rfc{n_layer + 1}", nn.ReLU()))
            network.append((f"dfc{n_layer + 1}", nn.Dropout(p=dropout)))
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            x = pyc_to_torch(
                x, s_indices=self.observation_keys, a_indices=self.action_keys
            )
            x = self.forward(x)
            return torch_to_pyc(x, indices=self.action_keys)
...
class RNN(myModule):
    def __init__(
        ...
        **_,
        ...
        self.input_size = base_in_size + (2 if a2c and feed_state else 0)
        self.hidden_size = hidden_size
        self.out_size = len(action_keys) + int(a2c)
        ...
        self.layers.append(
            GRULayer(self.input_size, hidden_size, hidden_bias, norm=norm)
        )
        init_states = [nn.Parameter(torch.zeros(hidden_size))]
        for n_layer in range(num_layers):
            if n_layer < num_layers // 2:
                latest_layer = GRULayer(last_num, last_num * 2, norm=norm)
        ...
        if self.out_size:
            self.layers.append(nn.Linear(hidden_size, self.out_size, bias=out_bias))
            self.out_act = out_act

        if self.learn_init:
            self.initial_states = nn.ParameterList(init_states[: num_layers + 1])

        self.observation_keys = state_keys
        self.action_keys = action_keys
        self.hypothesizer = None
        self.melded_model = None
        self.action_dims = (len(action_keys),)
        self.env = None
    ...
    def forward(self, x, h_0=None):
        if isinstance(x, dict):
            with torch.no_grad():
                ...
                x, _ = self.forward(x.unsqueeze(0).unsqueeze(0), h_0)
                return torch_to_pyc(x, indices=self.action_keys)
        ...
        if h_0 is None and self.learn_init:
            ...
            h_0 = [None] * (self.num_layers + 1)
        elif not isinstance(h_0, list) or len(h_0) < self.num_layers + 1:
            raise ValueError("h_0 must be a list with one hidden tensor per RNN layer")

        outputs = x
        final_hiddens = []
        for i in range(len(self.layers) - int(self.out_size != 0)):
            outputs, h_n = self.layers[i](outputs, h_0[i])
...
class Hypothesizer(myModule):
    ...
    def forward(
        self, prev_actions_one_hot, prev_rewards, current_states, goal, hidden=None
    ):
        """
...
class WorldModel(myModule):
    def __init__(
        ...
        self.net = nn.Sequential(
            nn.Linear(state_dim + total_embed_size + goal_dim, hidden_size),
            nn.ReLU(),
    ...
    def forward(self, state, action_one_hot, goal):
        ...
        assert action_one_hot.size(1) == self.total_action_dim, (
            f"action_one_hot should be (B*M, {self.total_action_dim}) but is {action_one_hot.shape}"
        )
        assert state.size(1) == self.state_dim, (
            f"state should be (B*M, {self.state_dim}) but is {state.shape}"
        )
        assert goal.size(1) == self.goal_dim, (
            f"goal should be (B*M, {self.goal_dim}) but is {goal.shape}"
        )
        ...
        act_embeds = torch.cat(act_embeds_list, dim=-1)
        assert act_embeds.size(-1) == sum(self.action_embedding_dim), (
            f"act_embeds should be (B*M, {sum(self.action_embedding_dim)}) but is {act_embeds.shape}"
        )

        inp = torch.cat([state, act_embeds, goal], dim=1)
        assert (
            inp.size(-1)
            == self.state_dim + sum(self.action_embedding_dim) + self.goal_dim
        ), (
            f"inp should be (B*M, {self.state_dim + sum(self.action_embedding_dim) + self.goal_dim}) but is {inp.shape}"
        )
        out = self.net(inp)
        assert out.size(-1) == self.state_dim + 1, (
            f"out should be (B*M, {self.state_dim + 1}) but is {out.shape}"
        )
        next_state = out[:, :-1]
        reward = out[:, -1]
...
class MeldedHypothesizerPlanner(myModule):
    def __init__(
        ...
        self.include_state_ins_for_actions = (
            include_state_ins_for_actions or permute_hypotheses
        )
    ...
    def forward(
        ...
        # shape checking
        assert prev_actions_one_hot.shape == (
            B,
            self.M,
            sum(self.action_dims),
        ), (
            f"prev_actions_one_hot should be (B, M, SumOfActionDims) but is {prev_actions_one_hot.shape}"
        )
        assert prev_rewards.shape == (
            B,
            self.M,
        ), f"prev_rewards should be (B, M) but is {prev_rewards.shape}"
        assert mode_tensor.shape == (
            B,
            1,
        ), f"mode_tensor should be (B, 1) but is {mode_tensor.shape}"
        assert current_states.shape == (
            B,
            self.M,
            self.state_dim,
        ), f"current_states should be (B, M, S) but is {current_states.shape}"
        assert goals.shape == (
            B,
            self.goal_dim,
        ), f"goals should be (B, G) but is {goals.shape}"

        assert torch.unique(mode_tensor).numel() <= 2, (
            f"mode_tensor should be -1 or 1 but is {torch.unique(mode_tensor)}"
        )
        assert torch.unique(mode_tensor).min() in [0, 1] and torch.unique(
            mode_tensor
        ).max() in [
            0,
            1,
        ], f"mode_tensor should be 0 or 1 but is {torch.unique(mode_tensor)}"
        ...
        for i, dim in enumerate(self.action_dims):
            ...
            assert torch.all(factor_one_hot.sum(dim=-1) == 1) or torch.all(
                factor_one_hot.sum(dim=-1) == 0
            ), f"factor_one_hot should be one hot but is {factor_one_hot.sum(dim=-1)}"
            embed = self.action_embeddings[i](factor_one_hot)
            act_embeds_list.append(embed)
            start_idx = end_idx
        ...
        assert act_embeds.shape == (
            B,
            self.M,
            sum(self.action_embedding_dim),
        ), f"act_embeds should be (B, M, num_factors*emb_dim) but is {act_embeds.shape}"
        ...
        rnn_input = torch.cat(
            [act_flat, rew_flat, mode_tensor, goals, states_flat], dim=1
        )
        assert rnn_input.shape == (
            B,
            self.input_size,
        ), f"rnn_input should be (B, InputSize) but is {rnn_input.shape}"
        ...
        assert values.shape == (
            B,
            self.M,
        ), f"values should be (B, M) but is {values.shape}"
    ...
    def get_standard_logits(self, rnn_out, states, goals):
        ...
        assert not self.is_factored, (
            "Called get_standard_logits but model is in Factored mode"
        )
        B = rnn_out.shape[0]
    ...
    def get_entity_logits(self, rnn_out, states, goals):
        ...
        """
        assert self.is_factored, (
            "Called get_entity_logits but model is in Standard mode"
        )
        B = rnn_out.shape[0]
        if len(states.shape) == 2:
            states = states.unsqueeze(1).expand(-1, self.M, -1)
    ...
    def get_relation_logits(self, rnn_out, states, goals, chosen_entities):
        ...
        """
        assert self.is_factored, (
            "Called get_relation_logits but model is in Standard mode"
        )
        B = rnn_out.shape[0]
...
class Orchestrator(myModule):
    def __init__(
        ...
        self.wm_optimizer = optim.Adam(
            ...
            lr=world_model_config.wm_lr,
            weight_decay=world_model_config.wm_weight_decay,
        )
    ...
    def policy_parameters(self):
        ...
        if self.use_melded_mode:
            ...
            return (
                ...
                + (
                    list(self.hypothesizer.parameters())
                    if not self.disable_hypo
                    else []
                )
                + (
                    list(self.aux_model.parameters())
                    if self.aux_model is not None
                    else []
                )
            )
    ...
    def planner_sample_norm(self, logits):
        ...
        assert len(logits.shape) == 3, (
            f"logits should be (B, M, A) but is {logits.shape}"
        )
        ...
        if planner:
            ...
            if not gumbel:
                action_indices = torch.multinomial(
                    F.softmax(action_logits, dim=-1), num_samples=1
                ).squeeze(-1)
                action_one_hot = F.one_hot(
                    action_indices, num_classes=action_logits.shape[-1]
                ).to(dtype=action_logits.dtype)
            else:
    ...
    def sample_action(self, rnn_out, states, goals, planner=True, gumbel=True):
        ...
        if not planner and self.permute_hypotheses:
            # 1. Frontier Sampling
            frontier_logits = self.melded_model.get_frontier_logits(
                rnn_out, states, goals
            )  # (B, M, M)
            parent_one_hot, parent_indices, _ = self.sample_subroutine(
                frontier_logits, planner=False, gumbel=True
            )
        ...
        if len(self.action_dims) > 1:
            ...
            entity_logits = self.melded_model.get_entity_logits(
                rnn_out, states_permuted, goals
            )
            entity_one_hot, entity_indices, new_entity_logits = self.sample_subroutine(
                entity_logits, planner=planner, gumbel=True
            )
            relation_logits = self.melded_model.get_relation_logits(
                rnn_out, states_permuted, goals, entity_one_hot
            )
            relation_one_hot, relation_indices, new_relation_logits = (
                self.sample_subroutine(relation_logits, planner=planner, gumbel=True)
            )
            ...
            if planner:
                new_logits_concatenated = torch.cat(
                    [new_entity_logits, new_relation_logits], dim=-1
                )
            else:
                new_logits_concatenated = torch.cat(
                    [entity_logits, relation_logits], dim=-1
                )

            return (
                one_hot_concatenated,
                indices_list,
                new_logits_concatenated,
                parent_one_hot,
            )
        else:
            ...
            policy_one_hot, policy_indices, new_policy_logits = self.sample_subroutine(
                policy_logits, planner=planner, gumbel=True
            )

            return (
                policy_one_hot,
                [policy_indices],
                (new_policy_logits if planner else policy_logits),
                parent_one_hot,
            )
    ...
    def diversity_loss(self, logits, in_states_actions, hypo_mask=None):
        ...
        assert logits.shape[-1] == self.total_action_dim, (
            f"logits should have last dim {self.total_action_dim} but is {logits.shape[-1]}"
        )
        assert len(logits.shape) == 3, (
            f"logits should be (B, M, {self.total_action_dim}) but is {logits.shape}"
        )
        assert hypo_mask is None or hypo_mask.shape == (
            logits.shape[0],
            1,
        ), f"hypo_mask should be (B,) but is {hypo_mask.shape}"
        assert len(in_states_actions.shape) == 3, (
            f"in_states_actions should be (B, M, {144 + self.total_action_dim}) but is {in_states_actions.shape}"
        )
        ...
        for dim in self.action_dims:
            ...
            kl_matrix = F.kl_div(log_p2, log_p1, reduction="none", log_target=True).sum(
                -1
            )

            if hypo_mask is not None:
                # Mask out invalid pairs
                mask_matrix = hypo_mask.unsqueeze(2) * hypo_mask.unsqueeze(
                    1
                )  # (B, 1, 1)
                diagonal_mask = ~torch.eye(
                    M, device=kl_matrix.device, dtype=torch.bool
                ).unsqueeze(0)  # (1, M, M)
            ...
            assert disagreement.shape == (), (
                f"disagreement should be () but is {disagreement.shape}"
            )
    ...
    def run_hypothesizer_loop(self, init_states, goal_tensor, h_t=None):
        """
        ...
        self.env.goal_repr_mode

        for step in range(12):
            policy_logits, h_new = self.hypothesizer(
                prev_actions_one_hot, prev_rewards, curr_states, goal_tensor, h_t
            )
            h_t = h_new
            logits = policy_logits

            div_loss = self.diversity_loss(
                logits, torch.cat([curr_states, prev_actions_one_hot], dim=-1)
            )
            ...
            wm_loss_val = F.mse_loss(pred_next_struct, gt_next_struct) + F.mse_loss(
                pred_rewards, gt_rewards
            )
    ...
    def compute_wm_loss(
        ...
        if profiler:
            profiler.stop("hypo_alg_step")

        if profiler:
            profiler.start("hypo_wm_fwd")
        pred_next_struct, pred_rewards = self.world_model(
            states_flat,
        ...
        if profiler:
            profiler.stop("hypo_wm_fwd")

        if profiler:
            profiler.start("hypo_wm_loss_calc")
    ...
    def run_melded_loop(
        ...
        assert in_states.shape == (
            B,
            self.state_dim,
        ), f"in_states should be (B, {self.state_dim}) but is {in_states.shape}"
        assert goal_tensor.shape == (
            B,
            self.goal_dim,
        ), f"goal_tensor should be (B, {self.goal_dim}) but is {goal_tensor.shape}"

        if h_t is None:
            assert rewards is None and actions is None, (
                "rewards and actions must be None if h_t is None"
            )

            curr_states = in_states.unsqueeze(1).expand(B, M, -1)
            prev_actions_one_hot = torch.zeros(
                B, M, self.total_action_dim, device=device
            )
            prev_rewards = torch.zeros(B, M, device=device)
            ...
            assert rewards.shape == (
                B,
                1,
            ), f"rewards should be (B, 1) but is {rewards.shape}"
            assert actions.shape == (
                B,
                self.total_action_dim,
            ), (
                f"actions should be ({B}, {self.total_action_dim}) but is {actions.shape}"
            )
    ...
    def melded_hypothesize(
        ...
        final_hiddens = [
            torch.zeros((B, self.hypo_config.hidden_size), device=device)
            for _ in range(len(self.melded_model.rnn))
        ]

        for step in range(10):
            if profiler:
                profiler.start("hypo_nn")
            ...
            if profiler:
                profiler.stop("hypo_nn")

            # Sampling / Action Selection
            # Hyp Mode: Sample independently
            # Plan Mode: Average and sample
            if profiler:
                profiler.start("hypo_sample")
            # Note: sample_action now returns parent_one_hot for Permutation
            one_hot_actions, action_indices_list, policy_logits, parent_one_hot = (
                self.sample_action(rnn_out, curr_states, goal_tensor, planner=False)
            )
            if profiler:
                profiler.stop("hypo_sample")
            ...
            assert policy_logits.shape == (
                ...
                f"policy_logits should be (B, M, {self.total_action_dim}) but is {policy_logits.shape}"
            )
            assert one_hot_actions.shape == (
                B,
                M,
                self.total_action_dim,
            ), (
                f"one_hot_act should be (B, M, {self.total_action_dim}) but is {one_hot_actions.shape}"
            )

            # Div Loss
            if profiler:
                profiler.start("hypo_div")
            div_loss = self.diversity_loss(
                policy_logits,
                torch.cat(
                    [
            ...
            total_div_loss += div_loss
            if profiler:
                profiler.stop("hypo_div")

            if profiler:
                profiler.start("hypo_wm_step")
            # wm_loss uses curr_states_permuted
            wm_loss_val, gt_next_struct, gt_rewards = self.compute_wm_loss(
                curr_states_permuted,
                one_hot_actions,
                goal_tensor,
                action_indices_list,
            ...
            any_goal_reached = (
                (prev_rewards > 0.99).any(dim=1, keepdim=True).float()
            )  # (B, 1)
            should_switch = (any_goal_reached > 0.5) & (
                mode_tensor > 0.5
            )  # (B, 1) boolean
    ...
    def melded_forward(
        ...
        curr_goals.size(0)
        # Melded Logic
        rnn_out, values, h_final, div_loss, wm_loss = self.run_melded_loop(
            curr_states, curr_goals, rewards, actions, hidden, profiler=profiler
        )
    ...
    def forward(
        ...
        raise NotImplementedError
        if (
            hypothesizer_state is None or self.hypothesize_always
        ):  # Run Hypothesizer Loop
            h_final, div_loss, wm_loss = self.run_hypothesizer_loop(
                curr_states, curr_goals
            )
        else:
        ...
        if self.planner.feed_state:
            model_input = torch.cat([curr_states, curr_goals, rewards, actions], dim=-1)
        else:
        ...
        return (logits, val, planner_hidden, h_final, div_loss, wm_loss)

    def train_world_model_batch(
        self,
        states,
        actions_one_hot,
        goals,
        ...
        assert rewards_target.shape == (B,), (
            f"rewards_target should be ({B},) but is {rewards_target.shape}"
        )
        assert next_states_target.shape == (
            B,
            self.state_dim,
        ), (
            f"next_states_target should be ({B}, {self.state_dim}) but is {next_states_target.shape}"
        )
        assert actions_one_hot.shape == (
            B,
            self.total_action_dim,
        ), (
            f"actions_one_hot should be ({B}, {self.total_action_dim}) but is {actions_one_hot.shape}"
        )
        assert goals.shape == (
            B,
            self.goal_dim,
        ), f"goals should be ({B}, {self.goal_dim}) but is {goals.shape}"

        pred_next_struct, pred_rewards = self.world_model(
            states, actions_one_hot, goals
        )
        loss = F.mse_loss(pred_next_struct, next_states_target) + F.mse_loss(
            pred_rewards, rewards_target
        )
...
class TreeSearcher(myModule):
    ...
    def _setup_rnn(self, num_layers, hidden_size, norm=False):
        ...
        input_size = (
            self.config.M * (2 + self.config.action_embedding_dim)
    ...
    def _setup_action_heads(self, hidden_size, dropout=0.0, norm=False):
        if len(self.config.action_dims) == 1:
            ...
            inp_dim = (
                hidden_size
                + self.config.M * self.config.action_dims[0]
                + self.config.M * self.config.state_embedding_dim
                + self.config.goal_embedding_dim
            )
            self.relation_mlp = nn.Sequential(
        ...
        try:
            ...
            try:
                ...
                raise AttributeError(
                    f"{self} has no attribute {name} nor does the config object"
                )
    ...
    def forward(
        ...
        assert prev_values.shape == (
            B,
            self.M,
        ), f"prev_values should be (B, M) but is {prev_values.shape}"
        assert current_states.shape == (
            B,
            self.M,
            self.state_dim,
        ...
        assert act_embeds.shape == (
            B,
            self.M,
            self.total_action_embedding_dim,
        ), (
            f"act_embeds should be (B, M, total_action_emb_dim) but is {act_embeds.shape}"
        )
        ...
        # Concatenate: (B, M*TotalEmbed + M + 1 + G + M*S)
        rnn_input = torch.cat([act_flat, rew_flat, val_flat, goals, states_flat], dim=1)
        assert rnn_input.shape == (
            B,
            self.input_size,
        ), f"rnn_input should be (B, InputSize) but is {rnn_input.shape}"
    ...
    def sample_action(self, rnn_out, states, goals, gumbel=True):
        ...
        if len(self.action_dims) > 1:
            assert len(self.action_dims) == 2
            entity_logits = self.get_entity_logits(rnn_out, states_permuted, goals)
            entity_one_hot, entity_indices, new_entity_logits = self.sample_subroutine(
                entity_logits, gumbel=True
            )
            relation_logits = self.get_relation_logits(
                rnn_out, states_permuted, goals, entity_one_hot
            )
            relation_one_hot, relation_indices, new_relation_logits = (
                self.sample_subroutine(relation_logits, gumbel=True)
            )
            ...
            policy_logits = self.get_standard_logits(rnn_out, states_permuted, goals)
            policy_one_hot, policy_indices, new_policy_logits = self.sample_subroutine(
                policy_logits, gumbel=True
            )
            policy_indices = [policy_indices]

        value = self.get_value(rnn_out, states_permuted, goals, policy_one_hot)
        return (
            policy_one_hot,
            policy_indices,
            new_policy_logits,
