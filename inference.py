...
from new_pipeline.models import Orchestrator
from new_pipeline.env import NewPipelineEnv
from new_pipeline.config import SHAPES, RELS, ACTION_KEYS
from new_pipeline.train import initialize_from_saved, ModelTrainConfig, NewPipelineTrainer
from new_pipeline.data_prep import get_action_from_factorized
from evaluation import simple_goal_sequencessness_elaborate, SHAPE_MAP, REVERSE_SHAPE_MAP, brick_connectedness
from plotting import plot_sequences
...
class InstrumentedOrchestrator(Orchestrator):
    ...
    def forward(self, curr_states, curr_goals, rewards=None, actions=None, planner_hidden=None, hypothesizer_state=None, profiler=None):
        ...
        return super().forward(curr_states, curr_goals, rewards, actions, planner_hidden, hypothesizer_state, profiler)
...
def simple_goal_sequencessness_multi_head(goals, grids, stable_blocks, M=1):
    ...
    for i, step_actions in enumerate(goals):
        ...
        mask = (grid_unique != present) & (grid_unique != SHAPE_MAP[stable_block]) & (grid_unique != 0)
...
def run_inference(args):
    ...
    from new_pipeline.train import load_from_dirname, load_test_data
    ...
    env = NewPipelineEnv(test_data, args.batch_size, test_base, device=device, 
                         close_rewards=args.close_rewards, goal_repr=args.goal_repr, 
                         state_repr=args.state_repr, action_repr=args.action_repr)
    
    # 2. Initialize Model
    from new_pipeline.train import HypoConfig, PlannerConfig, WorldModelConfig
    ...
    batch_hypo_traces = [[] for _ in range(batch_size)] # List of list of M actions (the hypothesis rollout actions)
    batch_actions_seq = [[] for _ in range(batch_size)] # List of planner actions (strings)
    ...
    for step in range(max_steps):
                     ...
                     for t in range(0, num_hypo_steps, step_size):
                         if is_factored:
                             ...
                             for m in range(len(ent_idxs)):
                                 a_str = get_action_from_factorized(ent_idxs[m], rel_idxs[m])
                                 if a_str:
                         ...
                         if mask_idx < len(orchestrator.step_hypothesis_masks):
                             mask_val = orchestrator.step_hypothesis_masks[mask_idx][b].item()
        ...
        # Sample
        one_hot_actions, action_indices_list, policy_logits, _ = orchestrator.sample_action(
        ...
        # Collect data for active episodes
        current_hypo_states = orchestrator.step_hypothesis_states[-1] # (B, M, S)
        current_hypo_logits = orchestrator.step_hypothesis_logits[-1] # (B, M, A)
        ...
        for i in range(batch_size):
            if active_episodes[i]:
                ...
                # Hypo data
                # batch_hypo_states[i].append(current_hypo_states[i].clone())
                # batch_hypo_logits[i].append(current_hypo_logits[i].clone())
    ...
    print(f"Correct grids: {num_correct}/{num_total} ({100*num_correct/num_total:.2f}%)" if num_total > 0 else "No episodes collected.")
    ...
    if len(collected_hypo_sequences) > 0:
        ...
        sequences = simple_goal_sequencessness_multi_head(collected_hypo_sequences, collected_grids, collected_stable_blocks, M=args.num_hypo)
        ...
        for seq in collected_planner_sequences:
            ...
            for act_str in seq:
                if act_str:
                    ...
                    if fmt: ep_fmt.append([fmt]) # Wrap in list to simulate M=1? 
    ...
    if len(collected_hypo_sequences) == 0 and len(collected_planner_sequences) == 0:
        ...
        if planner_sequences is not None:
            plot_sequences(planner_sequences, path=save_path / "sequenceness_planner.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stats_file", type=str, default=None, help="File to append stats to instead of plotting")
