...
from utils import weighted_sample_without_replacement, get_start_relations, get_pairwise_relations
...
class ExhaustiveSampler:
    ...
    def sample(self, sample_size):
        if sample_size > len(self.current_grids):
            ...
            self.generator.shuffle(self.current_grids)
            return [self.grids[i] for i in ret_indices] + self.sample(sample_size - len(ret_indices))
        else:
...
class BatchedBrickEnvironment:
    ...
    def get_brick_weights(self):
        if np.mean(self.last_few_performances) > 0.78 and self.cur_phase < 4 and len(self.last_few_performances) == 100:
    ...
    def make_input_tensor(self, current_grid, goal_grid):
        if self.p_mode == "conv":
            current_grid = torch.from_numpy(current_grid.copy()).to(self.device).to(dtype=torch.float32).unsqueeze(0)
            goal_grid = torch.from_numpy(goal_grid.copy()).to(self.device).to(dtype=torch.float32).unsqueeze(0)
            ...
            return torch.from_numpy(np.concatenate((goal_grid.flatten(), current_grid.flatten()))).to(dtype=torch.float32)
    ...
    def _initialize_all_environments(self):
        ...
        if not hasattr(self, 'manual_init') or not self.manual_init:
            goal_types = Counter(random.choices(range(len(self.goal_grids)), 
                                            weights=self.get_brick_weights() if (len(self.goal_grids) == 4) else [1/4]* len(self.goal_grids),
                                            k=self.batch_size))
            goal_grids = []
    ...
    def step(self, actions: torch.Tensor) -> Tuple[torch.Tensor, 
        ...
        for i in range(self.batch_size):
            ...
            if action_str is not None and action_is_right(goal_grid, current_grid, action_str):
                ...
                new_state = self.make_input_tensor(
                    new_grid, 
                    goal_grid
                )
    ...
    def replace_done_episodes(self, dones: torch.Tensor):
        ...
        for i in range(self.batch_size):
            if dones[i]:
                goal_grid = self.goal_grid[random.choices(range(len(self.goal_grids)))].sample(1)
    ...
    def get_batch_info(self, i) -> List:
        ...
        return {
                ...
                'num_shapes': len(np.unique(self.goal_grids_batch[i])[np.unique(self.goal_grids_batch[i]) != 0]),
