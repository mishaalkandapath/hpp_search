...
class SimpleProfiler:
    ...
    def start(self, name):
        ...
        with self.lock:
            with open("profiler_debug.log", "a") as f:
    ...
    def stop(self, name):
        ...
        with self.lock:
            if name in self.starts:
                ...
                self.counts[name] += 1
                with open("profiler_debug.log", "a") as f:
                    f.write(f"STOP {name} {elapsed}\n")
            else:
                with open("profiler_debug.log", "a") as f:
                    f.write(f"STOP FAIL {name}\n")

    def report(self):
        """Return a formatted string report of timings."""
        with self.lock:
            with open("profiler_debug.log", "a") as f:
                f.write(f"REPORT called. Keys: {list(self.timers.keys())}\n")
            report_lines = ["\n=== Profiler Report ==="]
            ...
            for key in sorted_keys:
                ...
                report_lines.append(
                    f"{key:<20} | Total: {total_time:.4f}s | Avg: {avg_time:.4f}s | Count: {count}"
                )
            report_lines.append("=======================\n")
            return "\n".join(report_lines)
