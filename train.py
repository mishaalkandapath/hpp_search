...
from training.models import HypoConfig, Orchestrator, PlannerConfig, WorldModelConfig
from misc.profiler import NoOpProfiler, SimpleProfiler
from misc.utils import load_from_dirname
...
class NewPipelineTrainer(ActorCriticTrainer):
    ...
    def __init__(
        ...
        self.optimizer = optim.AdamW(
            self.model.policy_parameters(), lr=config.lr, weight_decay=config.wd
        )
    ...
    def save(self, path, prefix="best"):
        ...
        if self.model.hypothesizer is None:
            try:
                torch.save(
                    self.model.melded_model.state_dict(), base / f"{prefix}_melded.pt"
                )
        ...
        try:
            ...
            if hasattr(self.model, "wm_optimizer"):
                torch.save(
                    self.model.wm_optimizer.state_dict(), base / f"{prefix}_wm_opt.pt"
                )
        except Exception as e:
            print(f"Error saving optimizers: {e}")

    def compute_advantage_loss_batched(self, episode_data: ExtendedEpisodeData):
        """
        ...
        assert episode_data.policy_logits_seq.size(-1) == sum(self.model.action_dims), (
            f"Policy logits shape: {episode_data.policy_logits_seq.shape}, Action dims: {self.model.action_dims}"
        )
        ...
        return (
            ...
            + self.config.beta_diversity * div_loss
            - self.config.beta_entropy * entropy_loss
            + aux_loss,
            {
    ...
    def run_episode_batch(self, inference=False, test=False):
        ...
        while active_episodes.any() and step_count < max_possible_steps:
            ...
            rnn_out, values, new_hidden, new_hypo_hidden, div_loss, wm_loss = (
                self.model(
                    states,
                    goals,
            ...
            if not inference:
                ...
                if active_mask.any():
                    ...
                    self.wm_buffer.append(
                        (b_states, b_actions, b_goals, b_next_states, b_rewards)
                    )
            ...
            if self.model.aux_task:
                ...
                if final_hidden_states is None:
                    ...
                    final_hidden_states = torch.where(
                        active_episodes.unsqueeze(-1), current_h, final_hidden_states
                    )
        ...
        actions_seq = torch.stack(
            actions_seq, dim=1
        )  # (batch, seq_len, total_action_dim)
    ...
    def evaluate(self, num_episodes=128, test=False):
        ...
        if not test and getattr(self.config, "save_inference_data", False):
            ...
            Path(__file__).resolve().parent / "inference.py"
            ...
            if self.config.cmd_args is not None:
                ...
                cmd.extend(
                    [
                        ...
                        str(c_args.dropout),
                        "--state_repr",
                        str(c_args.state_repr),
                        "--goal_repr",
                        str(c_args.goal_repr),
                        "--action_repr",
                        str(c_args.action_repr),
                        "--num_hypo",
                        str(c_args.num_hypo),
                    ]
                )
                ...
                cmd.extend(
                    [
                        "--d_hidden",
                        str(getattr(self.config, "n_hidden", 32)),
                        "--n_layers",
                        str(getattr(self.config, "n_layers", 1)),
                    ]
                )
            ...
            if free_gb < 2:
                print(
                    f"CRITICAL WARNING: Only {free_gb:.2f} GB of disk space remaining! Less than 2GB available."
                )
                print(
                    "Suspending process to prevent disk full errors. Run 'kill -CONT' to resume when space is cleared."
                )
...
def initialize_from_saved(orchestrator, ctd_from, melded):
    ...
    ckpt_files = [
        f for f in ctd_from.iterdir() if f.is_file() and f.name.endswith(".pt")
    ]
    ...
    if melded:
        ...
        if len(planner_files) > 1:
            ...
            best_planner_file = planner_files[0]
        ...
        if len(hypothesizer_file) > 1:
            best_hypothesizer_file = [f for f in hypothesizer_file if "best" in f.name][
                0
            ]
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_layers", type=int, required=True, help="Number of RNN layers"
    )
    ...
    parser.add_argument(
        "--data_method",
        choices=["regular", "all_shuffled", "no_test_in_train", "gen_flat_train"],
        default="gen_flat_train",
    )
    parser.add_argument("--state_repr", choices=["asis", "clarion"], default="clarion")
    parser.add_argument(
        "--goal_repr", choices=["asis", "clarion", "pixel"], default="pixel"
    )
    parser.add_argument(
        "--action_repr", choices=["standard", "factored"], default="standard"
    )
    ...
    parser.add_argument(
        "--save_inference_data",
        action="store_true",
        help="Save inference data every evaluate call",
    )
    ...
    signal.signal(
        signal.SIGVTALRM, siguser_handler
    )  # SIGUSR1 is sometimes taken by other processes/libraries, maybe user meant just a signal.
    ...
    # WandB
    run = wandb.init(
        project="brickworld-new-pipeline",
        name=args.run_name,
        config={
    ...
    train_base, val_base, test_base = (
        "data/processed/gen_grids/flattened_removed_train/",
        "data/processed/gen_grids/flattened_removed_val/",
        "/w/150/lambda_squad/misc/clarion_replay/data/processed/regular/test_data/test_stims",
    )
    ...
    trainer = NewPipelineTrainer(
        orchestrator, env, val_env, train_config, test_env=test_env, logger=run
    )
