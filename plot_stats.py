...
def main():
    ...
    parser.add_argument("--stats_file", type=str, required=True, help="Path to the pickle file containing stats.")
    parser.add_argument("--run_name", type=str, required=True, help="Run name, used for output directory.")
    parser.add_argument("--index", type=int, default=None, help="If provided, plot only the stat at this index (0-based). Otherwise, aggregate all.")
    parser.add_argument("--bounds", type=str, default=None, help="JSON string representing a list of lists of ints. Each sublist should be [start_index, end_index] (inclusive), e.g. '[[0, 5], [10, 15]]'.")
    ...
    if args.index is not None:
        if args.index < 0 or args.index >= len(stats_list):
            print(f"Error: index {args.index} out of bounds (0 to {len(stats_list) - 1}).")
            sys.exit(1)
        ...
        seq_list = [s.get("sequences") for s in stats_list if s.get("sequences") is not None]
        sequences = aggregate_sequence_dicts(seq_list) if seq_list else None

        plan_seq_list = [s.get("planner_sequences") for s in stats_list if s.get("planner_sequences") is not None]
