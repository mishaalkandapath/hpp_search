...
from batched_data_prep import make_grid_after_action, action_is_right, ACTION_KEYS, STATE_KEYS, make_mlp_dict
from .data_prep import encode_one_hot_pixel, get_factorized_action_indices, get_action_from_factorized
from .config import SHAPE_DICT, SHAPES, RELS, ACTION_KEYS
from collections import Counter
from evaluation import brick_connectedness
import random

class NewPipelineEnv(BatchedBrickEnvironment):
    def __init__(
                    self, *args, 
                    goal_repr="pixel", state_repr="asis", action_repr="standard", 
                    **kwargs
                ):
        """
    ...
    def make_input_tensor(self, current_grid, goal_grid):
        ...
        if self.goal_repr_mode == "pixel":
            ...
            g_tensor = torch.from_numpy(goal_grid.flatten()).float().to(self.device).to(torch.float32)
            ...
            g_tensor = make_mlp_dict_optimized(goal_grid).to(self.device).to(torch.float32)
        # State Encoding
        if self.state_repr_mode == "asis":
            s_tensor = torch.from_numpy(current_grid.flatten()).float().to(self.device).to(torch.float32)
            ...
            if np.any(current_grid):
                ...
                s_tensor = make_mlp_dict_optimized(current_grid).to(self.device).to(torch.float32)
            else:
    ...
    def _initialize_single_environment(self, idx: int, goal_grid: np.array):
        ...
        if idx >= len(self.current_grids):
            ...
            if self.test:
                self.grid_possibles.append(get_start_relations(goal_grid) + get_pairwise_relations(goal_grid))
        else:
            ...
            if self.test:
                self.grid_possibles[idx] = get_start_relations(goal_grid) + get_pairwise_relations(goal_grid)
        ...
        for i in indices:
            ...
            for _ in range(4):
                ...
                if not want_yes and len(shapes_in_grid) < 4 and random.random() < 0.33:
                    ...
                    for B_cand in others:
                        ...
                        if want_yes and connected:
                            ...
                            B_name = [k for k,v in SHAPE_DICT.items() if v == B_cand][0]
                            ...
                            if not connected:
                                ...
                                rel = random.choice(["left", "above", "right", "below"][:rels.index(A_id)] + ["left", "above", "right", "below"][rels.index(A_id) + 1:])
                                chosen_action_str = f"{B_name}_{A_name}_{rel}"
                                break
                ...
                if self.action_repr_mode == "standard":
                     ...
                     if chosen_action_str in ACTION_KEYS:
                         ent_idx, rel_idx = get_factorized_action_indices(chosen_action_str)
                         # ent_idx: 0-3
                         # rel_idx: 0-16
                         q_vec[ent_idx] = 1.0
                         q_vec[4 + rel_idx] = 1.0
            ...
            if all_good:
                ...
                labels_batch.append(torch.tensor(ls_for_env, device=self.device)) # (4,)
                
        return torch.stack(questions_batch), torch.stack(labels_batch), skip_indices.unsqueeze(-1).expand(-1, 4).to(questions_batch[0].device)
    ...
    def step(self, action_indices: list[torch.Tensor] | torch.Tensor):
        ...
        for i in range(bs):
            ...
            if action_str is not None and action_is_right(goal_grid, current_grid, action_str):
                ...
                new_state, _ = self.make_input_tensor(
                    new_grid, 
                    goal_grid
                )
        ...
        return torch.stack(states), torch.tensor(rewards, device=self.device, dtype=torch.float32), torch.tensor(dones, device=self.device, dtype=torch.bool)
...
for idx, (key, val_str) in enumerate(STATE_KEYS):
    ...
    match = re.match(r"^(input|target)_(mirror_L|half_T|vertical|horizontal)_(row|col)(\d+)", key)
...
def decode_clarion(tensor, target=True):
    ...
    shapes = {}
    key_pattern = re.compile(r"^(input|target)_(mirror_L|half_T|vertical|horizontal)_(row|col)(\d+)")
    for idx in nonzero_idxs:
        ...
        shape = match.group(2)
        if shape not in shapes: shapes[shape] = {}
    ...
    for shape in shapes:
        for i in range(1, 4):
            ...
            assert clean_grid[row - 1][col -1] == 0, f"Row {row}, col {col} is not empty: current map: {clean_grid}{shapes}"
